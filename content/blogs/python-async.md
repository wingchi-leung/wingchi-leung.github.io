...
**asyncio 是啥**  
`asyncio` 是 Python 标准库里的**异步 I/O 框架**。它提供三样东西：
1、 事件循环（单线程里调度协程）
2、`async`/`await` 语法、
3、以及一堆异步版的 API（如 `asyncio.sleep`、异步网络读写等）。

你写 `async def` 和 `await`时就是在用 asyncio 这一套模型：用协程描述「要做什么」，用事件循环在单线程里轮流推进这些协程，适合高并发的 I/O 场景（网络、文件、定时等）。不用 pip 安装，`import asyncio` 即可。
